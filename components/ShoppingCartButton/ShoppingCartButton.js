        ...
        <div className="flex flex-col">
            ...
            <div onMouseEnter={() => setOrdersHidden(false)} 
                ...
                <div className="absolute z-10 block w-96 top-0 right-0">
                    <ul className="rounded border-solid border border-white">
                        {orders.map(({id, amount, price, title, image}) => (
                            <li key={`item-${id}`} className="flex flex-row h-24 border-solid border-t-0 border border-white text-white font-sans bg-blue-500 pr-2 rounded">
